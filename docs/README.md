...
* 반환하기 버튼을 통해 사용자가 투입한 금액을 자판기에 남은 동전을 기준으로 최소한의 동전 갯수로 반환을 요청할 수 있다.
  ...
  * 💡 편의 사항
    * [x] 사용자의 금액이 0원일 시 경고창을 띄워 금액 충전을 유도하여 준다.
...
* `index.html`을 활용하지 않고 `components` 폴더의 템플릿들을 기준으로 화면을 작성한다.
* 사용자 입력 이벤트를 위임하여 등록을 간소화한다.
* 하위 컴포넌트를 설정할 수 있으며, 서로 상태를 전달할 수 있다.
...
* 프리코스 기간 동안 정말 고생하셨습니다! 이런 좋은 환경을 만들어주셔서 감사합니다 😊
