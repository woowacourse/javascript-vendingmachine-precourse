...
13. **상품구매**: 사용자로부터 투입 금액을 입력받는다.
14. **상품구매**: 사용자로부터 입력받은 투입 금액은 10원으로 나누어 떨어져야한다.
15. **잔돈계산**: 잔돈을 돌려줄 때 보유한 최소 개수의 동전으로 잔돈을 돌려준다.
16. **잔돈계산**: 잔돈을 반활할 수 없는 경우 반환 가능한 금액만 반환한다.
17. **공통**: 다른 탭으로 이동했다 돌아와도 기존 탭의 상태가 최근 상태로 유지되어야한다
