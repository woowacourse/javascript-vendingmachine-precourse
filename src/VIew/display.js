...
export const displayProductAddTab = (product) => {
  ...
  tr.append(
      ...
      elementCreatorWithClass(
          'td',
          ADD_TAB_CLASS.TABLE_TD_QUANTITY,
          product.quantity,
      ),
  );
...
export const displayPossessCoins = (item) => {
  for (const key in item.coins) {
    document.getElementById(
        COIN_TABLE_ID[key],
    ).innerHTML = `${item.coins[key]}개`;
  }
};
...
export const displayProductPurchaseTab = (product) => {
  const tr = `<tr id= ${product.id} class= ${PURCHASE_TAB_CLASS.PURCHASE_ITEM}>
                ...
                `;
  document
      .getElementById(PURCHASE_TAB_ID.PRODUCT_TABLE)
...
export const displayChangedProduct = (product, item) => {
  document.querySelector(
      `#${product.id} .${PURCHASE_TAB_CLASS.PRODUCT_QUANTITY}`,
...
export const displayReturnedCoins = (coins) => {
  for (const key in COIN_VALUE) {
    document.getElementById(
        PURCHASE_TAB_ID.COIN_TABLE[key],
    ).innerHTML = `${coins[key]}개`;
  }
};
